...
s2 = ewCovar(reshape(intc,(length(intc),1)),0.94)
d2 = Normal(0,sqrt(s2[1]))
VaR2 = -quantile(d2,0.05)


#Fit each stock return to a T distribution
m, s, nu, d3 = fit_general_t(intc)
VaR3 = -quantile(d3,0.05)

rets = copy(intc)
idx = convert(Int64,0.05*length(rets))
sort!(rets)
VaR4 = -rets[idx]

current = prices[1,"INTC"]
...
oPrices = CSV.read("Project/INTC.csv",DataFrame)
pReturns = return_calculate(oPrices,dateColumn="Date")
ointc = pReturns[!,"INTC"]

density(ointc,label="Out of Sample")
...
density(ointc,label="Out of Sample")
p4 = density!(intc, label="Historic Distribution")
...
cov = ewCovar(Matrix(returns[!,portfolio.Stock]),0.94)

current = prices[size(prices,1),portfolio.Stock]

nSim = 10000
sim = simulate_pca(cov,10000)
